...
//HE_face* Mesh3D::InsertFace_FromLoop(std::vector<HE_vert* >& vec_hv) {
...
//	if ((*iterVert)->id() == pvertices_list_->size()) //如果新添加的点真的得新的点，压如vector
//	{
//		pvertices_list_->push_back(hv);
//	}
//	else
...
void Mesh3D::WriteToStlFile(const char* fouts) {
	std::ofstream fout(fouts);

	fout << "solid 3" << "\n";
	const std::vector<HE_face *>& faces = *(get_faces_list());
	for (size_t i = 0; i < faces.size(); ++i)
	{
		//科学计数法
		fout << "  facet normal " << std::fixed << std::setprecision(4) << faces[i]->normal().x() << "  " << faces[i]->normal().y() << "  " << faces[i]->normal().z() << "\n";
		fout << "      outer loop" << "\n";
		fout << "         vertex " << std::fixed << std::setprecision(4) << faces[i]->vertices_[0]->position().x() << "  " << faces[i]->vertices_[0]->position().y() << "  " << faces[i]->vertices_[0]->position().z() << "\n";
		fout << "         vertex " << std::fixed << std::setprecision(4) << faces[i]->vertices_[1]->position().x() << "  " << faces[i]->vertices_[1]->position().y() << "  " << faces[i]->vertices_[1]->position().z() << "\n";
		fout << "         vertex " << std::fixed << std::setprecision(4) << faces[i]->vertices_[2]->position().x() << "  " << faces[i]->vertices_[2]->position().y() << "  " << faces[i]->vertices_[2]->position().z() << "\n";
		fout << "      endloop" << "\n";
		fout << "  endfacet" << "\n";
	}
	fout << "endsolid" << "\n";
...
bool Mesh3D::LoadFromSTLFile(const char* fins)
{
	...
	QFile file(filename);
	int n = 0, m = 0, k = 0; //存储取整后的nvv

	int a = 0, b = 0, c = 0;	//代表小数点后的4.5.6位
	...
	if (file.read(1) == "s")
	{
		...
		while (!inASCII.atEnd())
		{
			...
			else if (temp == "vertex")
			{
				...
				//数据格式化，保留小数点后6位
				nvv[0] = strVar.setNum(nvv[0], 'f', 6).toFloat();
				nvv[1] = strVar.setNum(nvv[1], 'f', 6).toFloat();
				nvv[2] = strVar.setNum(nvv[2], 'f', 6).toFloat();

				nvv[0] = getRound(nvv[0]);
				nvv[1] = getRound(nvv[1]);
				nvv[2] = getRound(nvv[2]);
				

				if (num++ < 20)
				{
					qDebug() << "vertex:" << nvv[0] << " " << nvv[1] << " " << nvv[2];
				}
...
void Mesh3D::UpdateMesh(void)
{
	...
	Unify(1.5);
	qDebug() << "bhelist=" << bheList->size() << "pedgeList=" << pedges_list_->size();
}
...
void Mesh3D::SetBoundaryFlag(void)
{
	...
	qDebug() << "setBoundflag bhelist=" << bheList->size();
}
...
//重置模型位置,注意检测model时用到了min 和max 故在unify中不应调用ComputeBoundingBox()
...
void Mesh3D::computeComponent()
{
	...
	for (size_t i = 0; i < bheList->size(); i++)
	{
		...
		do
		{
			...
			cur = cur->pvert_->pedge_;
		} while (cur != sta&&!cur->is_selected_);
...
//如果面片bloop是void，也即是不存在坏边，bhelist is void.单壳体
