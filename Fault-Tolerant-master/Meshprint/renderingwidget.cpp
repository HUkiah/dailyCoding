...
void RenderingWidget::ReadMesh()
{
	...
	if (filename.isEmpty())
	{
		//emit operatorInfo("Read Mesh Failed!");
		return;
	}
	//中文路径支持
...
void RenderingWidget::WriteMesh()
{
	...
	//ptr_mesh_->MntnMesh(byfilename.data());	//由Tri导出的STL
	ptr_mesh_->WriteToStlFile(byfilename.data());
}
	...
	if (holeMark)
	{
		...
		for (size_t i = 0; i != bl.size(); i++)
		{
			...
			for (int j = 0; j < bl[i].size(); j++)
			{
				glNormal3fv(bl[i][j]->start_->normal().data());
				glVertex3fv((bl[i][j]->start_->position()*scaleV).data());
				glNormal3fv(bl[i][j]->pvert_->normal().data());
				glVertex3fv((bl[i][j]->pvert_->position()*scaleV).data());
			}
			glEnd();
		}
	}
